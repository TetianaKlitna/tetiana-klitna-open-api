...
const current =
  "temperature_2m,wind_speed_10m,wind_direction_10m,weathercode,relative_humidity_2m";
const daily =
  "temperature_2m_max,temperature_2m_min,wind_speed_10m_max,wind_direction_10m_dominant,weathercode,relative_humidity_2m_max,relative_humidity_2m_min";
...
function getFetchDailyWeather(data) {
  ...
  for (let i = 0; i < nextDays.length; i++) {
    ...
    titleItem.innerHTML += `${getDayWeekStr(date)} ${shortFormatDate(
      date
    )} <br>${windDirectionStr} <br> ${maxWindSpeed[i]} ${unitWindSpeed}`;
    ulItem.append(titleItem);
  }
  ...
  for (let i = 0; i < nextDays.length; i++) {
    ...
    titleItem.innerHTML += `${getDayWeekStr(date)} ${shortFormatDate(
      date
    )} <br>💧: ${minHumidity[i]}/${maxHumidity[i]}${unitMinHumidity}`;
    ulItem.append(titleItem);
  }
}
...
function getFetchCurrentWeather(data) {
  ...
  let currWeatherDateContainer = document.getElementById(
